...
function DetailScreen({ city, shopName, onBack, stamps, onToggleStamp, saved, onToggleSave, visitedShops, onGotDirections, shopStampCounts }) {
      ...
      <ScrollView ref={scrollRef} style={{ flex: 1, backgroundColor: '#fff' }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" {...panResponder.panHandlers}>
        ...
        <View style={{ padding: 20 }}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 16 }}>
            ...
            <Text style={{ fontSize: 11, color: '#AAA' }}>{(shopStampCounts[city + '||' + shopName] || 0).toLocaleString()} stamps</Text>
          </View>
...
export default function App() {
  ...
  const [shopStampCounts, setShopStampCounts] = useState({});
 
  const loadStampCounts = async () => {
    const { data } = await supabase.from('stamps').select('shop_name, city');
    if (data) {
      const counts = {};
      data.forEach(s => {
        const key = s.city + '||' + s.shop_name;
        counts[key] = (counts[key] || 0) + 1;
      });
      setShopStampCounts(counts);
    }
  };
  ...
  if (!appReady || !fontsLoaded) return (
    <View style={{ flex: 1, backgroundColor: '#E8D9C0', justifyContent: 'center', alignItems: 'center' }}>
      <Text style={{ fontSize: 72, fontWeight: '700', color: '#1A0F07', letterSpacing: -2 }}>Roam</Text>
      <ActivityIndicator color="#8B4A00" style={{ marginTop: 20 }} />
    </View>
  );
  ...
  const renderContent = () => {
    ...
    if (screen === 'detail') return <DetailScreen city={city} shopName={shopKey} onBack={handleBack} stamps={stamps} onToggleStamp={handleToggleStamp} saved={saved} onToggleSave={handleToggleSave} visitedShops={visitedShops} onGotDirections={handleGotDirections} shopStampCounts={shopStampCounts} />;
